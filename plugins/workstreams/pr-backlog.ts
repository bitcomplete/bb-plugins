...
import { displayTitle, inboxSection, inboxVerb, prLifecycle, unitLifecycle } from "./workstreams.js";
...
export type BacklogEntry = { repo: string; pr: Pr; stale: boolean };
export type BacklogRow = BacklogEntry & {
  group: BacklogGroup; verb: string; action: PrimaryAction | null; local: Row | null;
...
export function prBacklog(entries: readonly BacklogEntry[], locals: readonly Row[], now: number): BacklogRow[] {
  ...
  const result = [...unique.values()].map((entry): BacklogRow => {
    ...
    return { ...entry, group, verb, action, local, parent };
  });
...
export function backlogMatches(row: BacklogRow, query: string): boolean {
  const haystack = `${row.repo} #${row.pr.number} ${row.pr.title} ${row.local?.effort ?? ""} ${row.verb}`.toLowerCase();
