...
import type { AdvancePreview } from "./bulk-advance.js";
...
async function setup(options: { remoteOnly?: boolean; mixedCase?: boolean; ready?: boolean; fork?: boolean; omitLaunchedThreadsFromList?: boolean } = {}) {
  ...
  const facts: AdvanceFacts = { prUrl: url, number: 42, title: pr.title, repo, headRefName: "abc-42-lookup", baseRefName: "main", headOid: HEAD, baseOid: BASE,
    ...
    mergeable: options.ready ? "MERGEABLE" : "CONFLICTING", needsPreparation: !options.ready, readiness: options.ready ? "ready" : "needs-attention",
    detail: options.ready ? "Approved and ready to merge" : "Resolve branch conflicts", unresolvedThreads: 0, checks: "passed", basePrNumber: null, approvalNotePending: false };
  ...
  return { harness, calls, spawn, send, facts, url, preview: async (prUrl = url) => await harness.callRpc("advance_preview", { prUrls: [prUrl] }) as AdvancePreview };
}
...
describe("bulk advance server integration", () => {
  ...
  it("verifies an already-ready fork without provisioning or starting a writer", async () => {
    ...
    const plan = await env.preview();
    expect(plan.jobs[0]).toMatchObject({ eligible: true, needsPreparation: false });
    await env.harness.callRpc("advance_start", { token: plan.token });
